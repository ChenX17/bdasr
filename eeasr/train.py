...
from eeasr.evaluate_tokens_per_batch import Evaluator
...
def train(config):
  ...
  with tf.Session(config=sess_config, graph=model.graph) as sess:
    ...
    def maybe_save_model():
      ...
      if config.train.eval_on_dev and step > 50000:  # Start cross validation only after 50k steps
        evaluator = Evaluator()
        evaluator.init_from_existed(config, model, sess)
        evaluator.translate(config.dev.feat_file_pattern,
                            config.dev.output_file + 'decode_result_step_' + '{}'.format(str(step)))
