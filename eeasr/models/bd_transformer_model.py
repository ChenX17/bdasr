...
class BD_TransformerModel(BaseModel):
    ...
    with self.graph.as_default():
      ...
      for i, device in enumerate(self._devices):
        with tf.device(device):
          ...
          pls_masked_y = tf.placeholder(
            dtype = tf.int32, shape=[None, 2, None],
            name='masked_dst_pl_{}'.format(i))
          src_pls.append(pls_batch_x)
          label_pls.append(pls_batch_y)
          masked_label_pls.append(pls_masked_y)
      self.src_pls = tuple(src_pls)
      self.label_pls = tuple(label_pls)
      self.masked_label_pls = tuple(masked_label_pls)
  ...
  def build_train_model(self, test=True, reuse=None):
    ...
    with self.graph.as_default():
      ...
      for i, (X, Y, MASKED_Y, device) in enumerate(
            zip(self.src_pls, self.label_pls, self.masked_label_pls, self._devices)):
        ...
        with tf.variable_scope(tf.get_variable_scope(),
          ...
          with tf.device(device_setter):
            ...
            MASKED_Y = tf.transpose(MASKED_Y, perm=[1, 0, 2])
            Y = tf.transpose(Y, perm=[1, 0, 2])
            decoder_output = self.decoder(utils.shift(MASKED_Y), encoder_output,
                                          is_training=True,
                                          reuse=i > 0 or None,
