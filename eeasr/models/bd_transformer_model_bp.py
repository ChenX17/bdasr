...
class BD_TransformerModel(BaseModel):
      ...
      for i, (X, Y, device) in enumerate(
        ...
        with tf.device(device):
          ...
          def true_fn():
            ...
            prediction = self.beam_search(enc_output, vocab_size=self._config.vocab_size, reuse=i > 0 or None)
          ...
          prediction = tf.cond(tf.greater(tf.shape(X)[0], 0),
                                      true_fn, false_fn)
  ...
  def beam_search(self, encoder_output, vocab_size, reuse):
    ...
    # [batch, beam]
    finished_scores = tf.ones([batch_size, beam_size]) * (-float("inf"))
    ...
    def step(flat_curr_scores, i, alive_seq, alive_log_probs, finished_seq,
     finished_scores, finished_flags, states, cache_qkv):
      ...
      curr_scores = topk_scores + tf.to_float(topk_finished) * -float("inf") # (batch, 2*beam)
      ...
      curr_scores = topk_scores + (1. - tf.to_float(topk_finished)) * -float("inf")
      ...
      #[batch, beam, length+1]
      finished_seq = tf.concat([finished_seq,tf.ones([batch_size, beam_size, 1], tf.int32)*3], axis=2)
      #[batch, 2, beam/2, length+1]
      finished_seq = tf.reshape(finished_seq, [batch_size, 2, beam_size/2, -1])
      #[batch, 2*beam]
      curr_scores = topk_scores + (1. - tf.to_float(topk_finished)) * -float("inf")
      #[batch, 2, 1.5*beam, length+1]
      curr_finished_seq = tf.concat([finished_seq, tf.reshape(topk_seq,[batch_size, 2, beam_size, -1])],axis=2)
      #[batch, 2, 1.5*beam]
      curr_finished_scores = tf.concat([tf.reshape(finished_scores, [batch_size, 2, -1]), tf.reshape(curr_scores,[batch_size, 2, beam_size])], axis=2)
      curr_finished_flags = tf.concat([tf.reshape(finished_flags, [batch_size, 2, -1]), tf.reshape(topk_finished, [batch_size, 2, beam_size])], axis=2)

      _, topk_indexes = tf.nn.top_k(curr_finished_scores, k=beam_size/2)
      topk_indexes_tmp = topk_indexes[:, 1, :] + 3*beam_size/2
      topk_indexes = tf.concat([tf.expand_dims(topk_indexes[:, 0, :],1),tf.expand_dims(topk_indexes_tmp,1)],axis=1)
      topk_indexes = tf.reshape(topk_indexes, [batch_size,beam_size])
      batch_pos_3 = compute_batch_indices(batch_size, beam_size)
      top_coordinates = tf.stack([batch_pos_3, topk_indexes], axis=2)

      curr_finished_seq = tf.reshape(curr_finished_seq, [batch_size, 3*beam_size, -1])
      finished_seq = tf.gather_nd(curr_finished_seq, top_coordinates)

      curr_finished_flags = tf.reshape(curr_finished_flags, [batch_size, 3*beam_size])
      finished_flags = tf.gather_nd(curr_finished_flags, top_coordinates)
      
      curr_finished_scores = tf.reshape(curr_finished_scores, [batch_size, 3*beam_size])
      finished_scores = tf.gather_nd(curr_finished_scores, top_coordinates)'''
    ...
    #max_indices = tf.to_int32(tf.argmax(finished_scores, axis=-1))
    #max_indices += tf.range(batch_size) * beam_size
    #finished_seq = tf.reshape(finished_seq, shape=[batch_size * beam_size, -1])
    #final_seq = tf.gather(finished_seq, indices=max_indices)
