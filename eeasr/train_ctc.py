...
from eeasr.dataloader import data_loader
from eeasr.models.base_model import BaseModel
from eeasr.models.transformer_model import TransformerModel
from eeasr.models.transformer_ctc_joint_model import CTC_joint_TransformerModel
...
def train(config):
  ...
  with tf.Session(config=sess_config, graph=model.graph) as sess:
    ...
    if tf.train.latest_checkpoint(config.model_dir):
      available_vars = utils.available_variables(config.model_dir)
      if available_vars:
    ...
    def train_one_step(batch):
      ...
      #target_batch = sparse_tuple_from([target_batch])
      #x_length = [feat_batch.shape[1]]
      x_length = [feat_batch.shape[1]] * feat_batch.shape[0]
      #y_length = target_batch.shape[1]
      logger.info("feat_batch_size: " + str(feat_batch.shape)
              + " label_batch_size: " + str(target_batch.shape))
      #sparse_target_batch = sparse_tuple_from([target_batch])
      feed_dict = data_loader.expand_feed_dict(
              {model.src_pls: feat_batch,
               model.label_pls: target_batch,
               model.x_len_pls: x_length})

              # model.y_len_pls: y_length})
               #model.sparse_label_pls: sparse_target_batch})
      #feed_dict.update({model.sparse_label_pls: sparse_target_batch })
      #feed_dict.update({model.seq_len_pls: sequence_length})
    ...
    for epoch in range(1, config.train.num_epochs+1):
      ...
      # Save model per epoch if config.train.save_freq is less or equal than
      # zero.
