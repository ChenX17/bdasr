...
import eeasr.dataloader.test_dataloader_240dim
import eeasr.models.base_model
import eeasr.models.transformer_model
...
class Evaluator(object):
    ...
    def init_from_config(self, config):
        ...
        if config.test.checkpoint:
            self.model.saver.restore(self.sess, os.path.join(config.model_dir,config.test.checkpoint))
        else:
            print(config.model_dir)
            self.model.saver.restore(self.sess, tf.train.latest_checkpoint(config.model_dir))
        self.data_reader = eeasr.dataloader.test_dataloader_240dim.DataReader(config)
    ...
    def beam_search(self, X):
        return self.sess.run(self.model.prediction, feed_dict=eeasr.dataloader.test_dataloader_240dim.expand_feed_dict({self.model.src_pls: X}))

    def loss(self, X, Y):
        return self.sess.run(self.model.loss_sum, feed_dict=eeasr.dataloader.test_dataloader_240dim.expand_feed_dict({self.model.src_pls: X, self.model.dst_pls: Y}))
...
if __name__ == '__main__':
    ...
    config = eeasr.dataloader.test_dataloader_240dim.AttrDict(yaml.load(open(args.config)))
    # Logger
