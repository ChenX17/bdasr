...
class DataLoader(object):
  ...
  def get_training_batches_with_buckets_using_scp(self, shuffle=True):
    ...
    for index,scp_path in enumerate(self.scp_files):
      ...
      for item in scp:
        uttid = item.split(' ')[0]
        scp_tr2l.append(uttid+'_tr2l'+' '+item.split(' ')[1])
        scp_tl2r.append(uttid+'_tl2r'+' '+item.split(' ')[1])

      if index == 0:
        #total_scp = scp
        total_scp = scp_tl2r + scp_tr2l
      else:
        total_scp += scp_tl2r + scp_tr2l
    # Shuffle all scp
    if shuffle:
    ...
    while True:
      ...
      caches[bucket_index][2].append(sudo_target_l2r)
      caches[bucket_index][3].append(sudo_target_r2l)
      caches[bucket_index][4].append(uttid_lr)
      caches[bucket_index][5] += 1

      if caches[bucket_index][5] >= self.batch_bucket_limit[bucket_index]:
        feat_batch, feat_batch_mask = self._create_feat_batch(caches[bucket_index][0])
        target_batch, target_batch_mask = self._create_bd_target_batch(caches[bucket_index][1],
        caches[bucket_index][2], caches[bucket_index][3], caches[bucket_index][4], self.dst2idx)
        # yield (feat_batch, feat_batch_mask, target_batch, target_batch_mask)
        #yield (feat_batch, target_batch, len(caches[bucket_index][0]))
        self._batch_queue.put((feat_batch, target_batch, len(caches[bucket_index][0])))
        caches[bucket_index] = [[], [], [], [], [], 0]
  ...
  def _create_bd_target_batch(self, sents, sents_l2r, sents_r2l, direction, phone2idx):
    # sents的数据是[word1 word2 ... wordn]
    indices = []
    for sent in sents:
      x = []
    ...
    for sent in sents_l2r:
      ...
      for word in (sent + [EOS]):
        ...
        if x_tmp == UNK_INDEX and word != UNK:
          logging.warn('=========[ZSY]x_tmp=UNK_INDEX, word=' + str(word.encode('UTF-8')))
      indices_l2r.append(x)

    indices_r2l = []
    for sent in sents_r2l:
      x = []
      ...
      for word in (sent + [EOS]):
        ...
        if x_tmp == UNK_INDEX and word != UNK:
          logging.warn('=========[ZSY]x_tmp=UNK_INDEX, word=' + str(word.encode('UTF-8')))
      indices_r2l.append(x)
    
    # Pad to the same length.
    batch_size = len(sents)
    #if random sample > 0.5, use true l2r label, sudo r2l label
    
    maxlen = max(max([len(s) for s in indices]),max([len(s) for s in indices_r2l]), max([len(s) for s in indices_l2r]))
    #maxlen = max([len(s) for s in indices])
    ...
    for i, x in enumerate(indices):
      ...
      if direction[i].split('_')[-1]=='tl2r':
        #logging.info('using tl2r')
        target_batch_l2r[i, :len(x)] = x
        target_batch_r2l[i, :len(indices_r2l[i])] = indices_r2l[i][:-1][::-1]+[3]
        target_batch_mask[i, :len(x)] = 0
      else:
        #logging.info('using tr2l')
        target_batch_l2r[i, :len(indices_l2r[i])] = indices_l2r[i]
    ...
    return new_target, target_batch_mask
