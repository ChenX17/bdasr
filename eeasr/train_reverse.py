...
from eeasr.evaluate_reverse import Evaluator
from eeasr.dataloader import data_loader_reverse
...
def train(config):
  ...
  data_reader = data_loader_reverse.DataReader(config=config)
  ...
  with tf.Session(config=sess_config, graph=model.graph) as sess:
    ...
    def train_one_step(batch):
      ...
      feed_dict = data_loader_reverse.expand_feed_dict(
    ...
    def maybe_save_model():
      ...
      #if config.train.eval_on_dev and step > 50000:  # Start cross validation only after 10k steps
