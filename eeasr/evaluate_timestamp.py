...
class Evaluator(object):
    ...
    def beam_search(self, X):
        return self.sess.run([self.model.prediction, self.model.atten_probs],
    ...
    def translate(self, src_path, output_path, batch_size):
        ...
        for X,uttids in self.data_reader.get_test_batches(src_path, batch_size):
            Y, atten_probs= self.beam_search(X)
            print(atten_probs[0].shape)
            sents = self.data_reader.indices_to_words(Y)
            assert len(X) == len(sents)
            for index, (sent, uttid) in enumerate(zip(sents, uttids)):
                print(uttid + '\t' + sent, file=fd)
                sent=sent.replace(' ', '')
                #print(uttid + '\t' + sent.encode('utf8'))
                sent_len = len(sent)
                atten_index = np.argmax(atten_probs[0][index, : ,:], axis=-1)
                atten_index = atten_index[:sent_len]
                print (atten_index, file=fd)
            count += len(X)
...
if __name__ == '__main__':
    ...
    config = eeasr.dataloader.test_data_loader.AttrDict(yaml.load(open(args.config)))
    # Logger
